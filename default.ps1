...
task ILMerge -depends Compile {
	$input_dlls = "$output_directory\IdentityServer3.Admin.EntityFramework.dll"

	Get-ChildItem -Path $output_directory -Filter *.dll |
		foreach-object {
			# Exclude IdentityServer.Core.dll as that will be the primary assembly
			if ("$_" -ne "IdentityServer3.Admin.EntityFramework.dll" -and 
			    "$_" -ne "Owin.dll") {
...
task CreateNuGetPackage -depends Compile {
	...
	New-Item $dist_directory\lib\net45 -Type Directory
	copy-item $output_directory\IdentityServer3.Admin.EntityFramework.* $dist_directory\lib\net45
