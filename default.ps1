...
task ILMerge -depends Compile {
	$input_dlls = "$output_directory\Thinktecture.IdentityServer3.Admin.EntityFramework.dll"
	...
	Get-ChildItem -Path $output_directory -Filter *.dll |
		foreach-object {
			...
			if ("$_" -ne "Thinktecture.IdentityServer3.Admin.EntityFramework.dll" -and 
			    "$_" -ne "Owin.dll") {
